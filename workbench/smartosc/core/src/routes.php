<?php
use Smartosc\Core\CoreServiceProvider;
use Illuminate\Support\Facades\Config;
$smartosc_packages = CoreServiceProvider::getSmartOscPackages();
    ...
    foreach($smartosc_packages as $package_name){
        ...
        if($package_routes && is_array($package_routes)){
            foreach($package_routes as $route_name =>  $route){
                $url    = $route['url'];
                $action = (isset($route['action']) && $route['action']) ? $route['action'] : 'Smartosc\Core\CoreController@getDefault';
        ...
        if($normalRoutes){
            foreach($normalRoutes as $route_name => $route){
                ...
                if(strpos($action, "\\") === false){
                    $action = 'Smartosc\\'.ucfirst($package_name).'\\Controllers\\'.$action;
                }
