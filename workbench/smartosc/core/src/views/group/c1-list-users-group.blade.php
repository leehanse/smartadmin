<div class="row upper-menu" style="height: 35px;">
    ...
    <div style="float:right;">
        <a id="delete-item" class="btn btn-danger users">{{ trans('core::all.delete') }}</a>
    </div>
</div>
...
<thead>
    <tr>
        ...
        <th style="width:200px;">{{ trans('core::users.username') }}</th>
        <th style="width:30px; text-align: center;">{{ trans('core::all.show') }}</th>
    </tr>
</thead>
<tbody>
    ...
    <tr>
        ...
        <td>&nbsp;{{ $user->username }}</td>
        <td style="text-align: center;">&nbsp;<a href="{{ URL::route('showUser', $user->getId()); }}">{{ trans('core::all.show') }}</a></td>
    </tr>
    @endforeach
</tbody>
</table>
...
<div class="row">
    ...
    <div class="col-lg-4">
        <button id="add-user" type="button" class="btn btn-primary">{{ trans('core::groups.add-user') }}</button>
    </div>
</div>
@endif
